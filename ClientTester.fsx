...
let tester _ = 
    let api = "Login" // "Login" "Register" "Tweet" "ReTweet" "Follow" "UnFollow" "Query"
    let id = "Admin007"
    ...
    let json = sprintf """{"api": "%s","auth":{"id":"%s","password":"%s"},"props":%s}"""
                    api id password loginProp
    // let json = sprintf """{"status": "%s","msg":"%s","content":%s}""" "200" "success" """[{"Jan": "Alexander"}]"""
    printfn "%s" json
    // printfn "%A" (JsonValue.Parse(json))?auth
    let server = remoteSystem.ActorSelection ("akka.tcp://TwitterClone@192.168.171.128:9001/user/APIHandler")
